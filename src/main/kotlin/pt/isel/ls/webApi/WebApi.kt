...
class WebApi(services: Services) {
    ...
    private fun requestHandler(withAuth: Boolean, req: Request, function: (Request) -> Response): Response {
        return try {
            ...
            val rsp = getError(e)
            println(rsp.message)
            Response(OK).body(Json.encodeToString(rsp))
        }
    }
    ...
    fun getRoutes() = routes(
        "users" bind POST to { r -> requestHandler(false, r, ::createUser) },
        "users" bind GET to { r -> requestHandler(false, r, ::getUsers) },
        "users/login" bind GET to { r -> requestHandler(false, r, ::login) },
        "users/{id}" bind GET to { r -> requestHandler(false, r, ::getUser) },
        "boards" bind POST to { r -> requestHandler(true, r, ::createBoard) },
        "boards" bind GET to { r -> requestHandler(false, r, ::getBoards) },
        "boards/{bId}" bind GET to { r -> requestHandler(false, r, ::getBoard) },
        "boards/{bId}" bind PUT to { r -> requestHandler(true, r, ::addUserToBoard) },
        "boards/{bId}/users" bind GET to { r -> requestHandler(false, r, ::getUsersOfBoard) },
        "lists" bind POST to { r -> requestHandler(true, r, ::createList) },
        "lists" bind GET to { r -> requestHandler(false, r, ::getLists) },
        "lists/{lId}" bind GET to { r -> requestHandler(false, r, ::getList) },
        "lists/{lId}/{bId}" bind DELETE to { r -> requestHandler(true, r, ::deleteList) },
        "cards" bind POST to { r -> requestHandler(true, r, ::createCard) },
        "cards" bind GET to { r -> requestHandler(false, r, ::getCards) },
        "cards/{cId}" bind GET to { r -> requestHandler(false, r, ::getCard) },
        "cards/{cId}" bind PUT to { r -> requestHandler(true, r, ::moveCard) },
        "cards/{cId}" bind DELETE to { r -> requestHandler(true, r, ::deleteCard) },
        "cards/archived/{bId}" bind GET to { r -> requestHandler(false, r, ::getCardsArchived) },
        "cards/archived/{cId}/{bId}" bind GET to { r -> requestHandler(false, r, ::getCardArchived) },
        "cards/archived/{cId}/{lId}/{bId}" bind PUT to { r -> requestHandler(true, r, ::archiveCard) },
        "cards/unarchive/{cId}/{lId}/{bId}" bind PUT to { r -> requestHandler(true, r, ::unarchiveCard) },
    ...
    private fun createUser(request: Request): Response {
        logRequest(request)
        val body = Json.decodeFromString<BodyFormat>(request.bodyString())
    ...
    private fun moveCard(request: Request): Response {
        ...
        return Response(OK)
            .header("content-type", "application/json")
            .body(Json.encodeToString(s.moveCard(newList, card, newCardIdx, oldList, board, user)))
    }
